...
, # path to pre-trained weights to load into networks
    ...
    algo_params=dict(
        meta_batch=32, # number of tasks to average the gradient across
        ...
        num_tasks_sample=6, # number of randomly sampled tasks to collect data for each iteration
        ...
        max_path_length=50, # max path length for this environment #200
        discount=0.99, # RL discount factor
        soft_target_tau=0.005, # for SAC target network update ##original 0.005
        policy_lr=3E-4, 
        qf_lr=3E-4,
        vf_lr=3E-4,
        context_lr=3E-4,
        reward_scale=10, # scale rewards before constructing Bellman update, effectively controls weight on the entropy of the policy ##original=5
        sparse_rewards=False, # whether to sparsify rewards as determined in env
        kl_lambda=0.1, # weight on KL divergence term in encoder loss original=0.1
